...
# 🚀 프로젝트 개요
구현된 16-bit Tiny Microprocessor는 5-stage Pipeline 구조를 기반으로 동작합니다. 명령어는 IF, ID, EX, MEM, WB 단계를 순차적으로 거치며, 산술 연산, 메모리 접근, 분기 명령어를 수행합니다. 파이프라인 구조에서 발생하는 Data Hazard를 해결하기 위해 Forwarding(Bypassing) 기법을 적용하였습니다. Execute 단계와 Decode 단계에 Forwarding Multiplexer를 추가하여 이전 명령어의 결과를 필요한 단계로 직접 전달하고, 불필요한 stall을 최소화하도록 설계하였습니다.
